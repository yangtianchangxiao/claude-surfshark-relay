...
async function exchangeCodeForTokens(authorizationCode, codeVerifier, state, proxyConfig = null) {
  ...
  try {
    ...
    const response = await axios.post(OAUTH_CONFIG.TOKEN_URL, new URLSearchParams(params), {
      headers: {
  ...
  try {
    ...
    if (!agent && (process.env.HTTP_PROXY || process.env.HTTPS_PROXY)) {
      ...
      try {
        ...
        logger.error('❌ Failed to parse system proxy URL for Setup Token:', proxyUrl, err.message)
      }
    } else if (agent) {
      logger.info(
