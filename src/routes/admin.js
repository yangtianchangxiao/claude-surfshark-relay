...
router.post('/claude-accounts/exchange-code', authenticateAdmin, async (req, res) => {
  try {
    ...
    // 构建代理配置 - 优先使用会话中的代理，否则使用环境变量
    let proxyConfig = oauthSession.proxy
    if (!proxyConfig && (process.env.HTTP_PROXY || process.env.HTTPS_PROXY)) {
      ...
      try {
        ...
        logger.error('❌ Failed to parse proxy URL:', proxyUrl, err.message)
        proxyConfig = null
      }
    }
